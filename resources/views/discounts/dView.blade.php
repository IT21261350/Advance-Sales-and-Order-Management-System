...
<head>
    ...
    <script>
        ...
        function redirectToAnotherPage() {
            window.location.href = "{{ route('oRder.Discount') }}";
        }
...
<nav class="navbar navbar-expand-lg">
    <div class="container">
        <a class="navbar-brand" onclick="redirectToAnotherPage1()">Discount</a>
...
<br>
    ...
    <button onclick="redirectToAnotherPage()" class="btn btn-primary">Add Discounts</button>
...
<table class="table">
    <thead>
        <tr>
            <th scope="col">Discount Label</th>
            <th scope="col">ID</th>
    ...
    <tbody>
        @foreach($discount as $row)
            <tr>
                ...
                <td>{{ $row->lowLimit }}</td>
                <td>
                    <button class="btn btn-success"><a id="bn6" href="{{ route('oRder.edit', ['discount' => $row->id]) }}"> Edit </a></button>
                    <form method="post" action="{{ route('oRder.delete', ['discount' => $row->id]) }}">
                        @csrf
                        @method('delete')
